...
for i in range(0, len(global_assets), 2):
    cols = st.columns(2)
    for col, (name, symbol) in zip(cols, global_assets[i:i+2]):
        with col:
            period_options = {
                "1D": ("1d", "5m"),
                "1W": ("5d", "15m"),
            ...
            if f'global_period_{symbol}' not in st.session_state:
                st.session_state[f'global_period_{symbol}'] = "1D"

            selected_period = st.selectbox(
                "Time Range",
                options=list(period_options.keys()),
                index=list(period_options.keys()).index(st.session_state[f'global_period_{symbol}']),
                key=f"global_select_{symbol}",
                label_visibility="collapsed"
            )

            st.session_state[f'global_period_{symbol}'] = selected_period
            period, interval = period_options[selected_period]
            ...
            try:
                ...
                if not df.empty:
                    ...
                    # Bitcoin uses larger numbers — format accordingly
                    is_btc = symbol == "BTC-USD"
                    fmt = f"${last_close:,.0f}" if is_btc else f"${last_close:.2f}"
                    fmt_high = f"${d_high:,.0f}" if is_btc else f"${d_high:.2f}"
                    fmt_low = f"${d_low:,.0f}" if is_btc else f"${d_low:.2f}"
                    fmt_chg = f"{change:,.0f} ({pct_change:+.2f}%)" if is_btc else f"{change:.2f} ({pct_change:+.2f}%)"
                    hover_fmt = '<b>Price</b>: $%{y:,.0f}<br><b>Time</b>: %{x}<extra></extra>' if is_btc else '<b>Price</b>: $%{y:.2f}<br><b>Time</b>: %{x}<extra></extra>'
                    prev_annotation = f"Prev: ${prev_close:,.0f}" if is_btc else f"Prev: ${prev_close:.2f}"

                    m1, m2, m3 = st.columns(3)
                    m1.metric(name, fmt, fmt_chg, delta_color="normal")
                    m2.metric("High", fmt_high)
                    m3.metric("Low", fmt_low)
                    ...
                    if selected_period == "1D":
                        fig.add_hline(
                            ...
                            line_color="gray",
                            opacity=0.5,
                            annotation_text=prev_annotation,
                            annotation_position="right",
                            annotation_font_size=9
                        )
